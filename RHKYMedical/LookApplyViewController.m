...
#import "LookApplyViewController.h"
...
@interface LookApplyViewController ()
{
...
//    NSArray *detailsArray16;
    LookDetailsViewController *lookDetailsViewController;
    NSInteger arrayCount;
    MBProgressHUD *HUD;
}
@end

@implementation LookApplyViewController
...
-(void)getList{
    ...
    NSDictionary *postDic = [NSDictionary dictionaryWithObjectsAndKeys:userid,@"userid",@"0",@"status",@"1",@"page_no",token,@"token",nil];
    ...
    [sessionManager POST:ApplyList_URL parameters:postDic progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        NSMutableArray *applyListArray = [[responseObjectDic objectForKey:@"body"]objectForKey:@"datalist"];
        NSData * data = [NSKeyedArchiver archivedDataWithRootObject:applyListArray];
        [[NSUserDefaults standardUserDefaults] setObject:data forKey:@"applyList"];
        
        if ([str isEqualToString:@"1"]) {
            HUD.mode= MBProgressHUDModeText;
            if (applyListArray.count == 0) {
                HUD.labelText = @"暂无手术室进入申请";
                [HUD show:YES];
                [HUD hide:YES afterDelay:1.5];
            }
            else{
            HUD.labelText = [NSString stringWithFormat:@"数据获取成功:共%lu条",(unsigned long)applyListArray.count];
            [HUD show:YES];
            [HUD hide:YES afterDelay:1.5];
...
{
    NSData * data = [[NSUserDefaults standardUserDefaults] objectForKey:@"applyList"];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!cell) {
        cell = [[[NSBundle mainBundle] loadNibNamed:@"DetailsTableViewCell" owner:self options:nil] objectAtIndex:0];
    }
    NSData * data = [[NSUserDefaults standardUserDefaults] objectForKey:@"applyList"];
    numberArray = [NSKeyedUnarchiver unarchiveObjectWithData:data];

    cell.RankLabel.text = [NSString stringWithFormat:@"%@",[numberArray[indexPath.row] objectForKey:@"operation_name"]];
    cell.timeLabel.text = [NSString stringWithFormat:@"%@",[numberArray[indexPath.row] objectForKey:@"operation_time"]];
    cell.addressLabel.text = [NSString stringWithFormat:@"%@",[numberArray[indexPath.row] objectForKey:@"operating_room_name"]];
    cell.peopleCountLabel.text = [NSString stringWithFormat:@"%@",[numberArray[indexPath.row] objectForKey:@"applytime"]];
    cell.patientNameLabel.text = [NSString stringWithFormat:@"%@",[numberArray[indexPath.row] objectForKey:@"status"]];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(nonnull NSIndexPath *)indexPath{
    ...
    NSData * data = [[NSUserDefaults standardUserDefaults] objectForKey:@"applyList"];
    numberArray = [NSKeyedUnarchiver unarchiveObjectWithData:data];
    lookDetailsViewController = [[LookDetailsViewController alloc]initWithNibName:@"LookDetailsViewController" bundle:nil];
    lookDetailsViewController.detailsDic = [numberArray objectAtIndex:indexPath.row];
    [self showViewController:lookDetailsViewController sender:nil];
